...
import com.snowplowanalytics.snowplow.badrows.SchemaValidationSpec.mockJsonValue
...
object PayloadGen {
  ...
  val selfDescribingEntity: Gen[Payload.SelfDescribingEntity] =
    Gen.alphaNumStr.map { line => Payload.SelfDescribingEntity(line, mockJsonValue) }

  val bqReconstructedEvent: Gen[Payload.BQReconstructedEvent] =
    Gen.listOf(selfDescribingEntity).map(entities => Payload.BQReconstructedEvent(SpecHelpers.ExampleEvent, entities))
}
